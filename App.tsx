...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const data = { 
      ...
      emdra_demands: demands, emdra_finances: finances, emdra_documents: documents, 
      emdra_students: students, emdra_teachers: teachers, emdra_staff_members: staffMembers 
    };
    Object.entries(data).forEach(([key, val]) => localStorage.setItem(key, JSON.stringify(val)));
  }, [appUsers, occurrences, attendance, demands, finances, documents, students, teachers, staffMembers]);
        ...
        <div className="bg-white p-12 rounded-[40px] shadow-2xl w-full max-w-md border border-slate-100">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              ...
              <input type="text" required className="w-full px-6 py-5 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold" value={loginForm.name} onChange={e => setLoginForm({...loginForm, name: e.target.value})} />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">PIN</label>
              <input type="password" required className="w-full px-6 py-5 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold" value={loginForm.pin} onChange={e => setLoginForm({...loginForm, pin: e.target.value})} />
            </div>
            <button className="w-full bg-slate-900 text-white py-6 rounded-2xl font-black uppercase tracking-widest shadow-xl active:scale-95">Acessar Painel</button>
          </form>
        </div>
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'ml-72' : 'ml-24'} p-12`}>
        {activeTab === 'dashboard' && <DashboardView stats={stats} insight={aiInsight} recentOccurrences={occurrences.slice(0, 5)} currentUser={currentUser} />}
...
const DashboardView: React.FC<any> = ({ stats, insight, recentOccurrences, currentUser }) => (
  <div className="space-y-12 animate-in fade-in duration-700">
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-10 items-start">
      <div className="xl:col-span-2 bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 overflow-hidden">
        <h3 className="text-xl font-black text-slate-800 mb-10 flex items-center gap-3"><PieChartIcon size={24} className="text-emerald-500" />Análise Disciplinar</h3>
      ...
      <div className="space-y-8">
        <div className="bg-slate-900 p-10 rounded-[40px] shadow-2xl text-white relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-6 relative z-10">
            ...
            <h4 className="font-black text-lg">IA Insights</h4>
          </div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500">
      <form onSubmit={addDemand} className="flex gap-4">
        <input type="text" className="flex-1 px-8 py-5 rounded-3xl bg-white border border-slate-200 shadow-sm outline-none font-bold" placeholder="O que precisa ser feito?" value={newDemand} onChange={e => setNewDemand(e.target.value)} />
        <button className="bg-slate-900 text-white px-10 rounded-3xl font-black uppercase hover:bg-slate-800 shadow-lg active:scale-95 transition-all"><Plus /></button>
      </form>
      <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden divide-y divide-slate-100">
        {demands.length === 0 && <div className="p-12 text-center text-slate-300 font-bold">Nenhuma pendência.</div>}
          ...
          <div key={d.id} className="p-8 flex items-center justify-between group">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-[10px] font-black text-slate-300 uppercase mt-1">{new Date(d.date).toLocaleDateString()}</p>
              </div>
            </div>
            <button onClick={() => setDemands(demands.filter((item: Demand) => item.id !== d.id))} className="p-3 text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={20} /></button>
          </div>
        ))}
      </div>
    </div>
...
const AttendanceView: React.FC<any> = ({ attendance, setAttendance, staff, teachers, showToast }) => {
  const [formData, setFormData] = useState({ personName: '', type: 'Falta', date: new Date().toISOString().split('T')[0] });
  const addRecord = (e: React.FormEvent) => {
    e.preventDefault();
    if(!formData.personName) return;
    const record: AttendanceRecord = { id: Math.random().toString(36).substr(2, 9), ...formData, roleOrSubject: '', description: '', isTeacher: true, type: formData.type as any };
      ...
      <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
        ...
        <form onSubmit={addRecord} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="space-y-2">
            ...
            <datalist id="names">{teachers.map((t: string) => <option key={t} value={t} />)}</datalist>
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tipo</label>
          ...
          <button className="bg-slate-900 text-white rounded-2xl font-black uppercase hover:bg-slate-800 h-[60px] self-end shadow-lg transition-all">Lançar</button>
        </form>
      </div>
    </div>
  );
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Saldo</p>
          <h4 className={`text-3xl font-black ${balance >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>R$ {balance.toFixed(2)}</h4>
        </div>
      </div>
      <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
        <h3 className="text-2xl font-black text-slate-800 mb-8">Lançamento de Caixa</h3>
...
const DocumentsView: React.FC<any> = ({ docs, setDocuments, showToast }) => {
  const [formData, setFormData] = useState({ name: '', category: 'Lei', linkOrBase64: '' });
  const addDoc = (e: React.FormEvent) => {
    e.preventDefault();
    if(!formData.name || !formData.linkOrBase64) return;
    const doc: Document = { id: Math.random().toString(36).substr(2, 9), ...formData, category: formData.category as any, type: 'Link' };
    setDocuments([doc, ...docs]);
    setFormData({ name: '', category: 'Lei', linkOrBase64: '' });
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500">
      <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
        ...
        <form onSubmit={addDoc} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <input placeholder="Título" required className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 font-bold" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
          ...
          <button className="bg-slate-900 text-white py-5 rounded-2xl font-black uppercase hover:bg-slate-800 shadow-xl transition-all">Adicionar</button>
        </form>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {docs.map((d: Document) => (
          <div key={d.id} className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 flex flex-col group hover:border-emerald-500/50 transition-all">
            <div className="flex justify-between mb-6">
              <FileSearch className="text-slate-400 group-hover:text-emerald-500" />
              <span className="text-[10px] font-black uppercase text-slate-400">{d.category}</span>
            </div>
            <h5 className="text-xl font-black mb-6 line-clamp-1">{d.name}</h5>
            <div className="flex gap-4">
              <a href={d.linkOrBase64} target="_blank" rel="noopener noreferrer" className="flex-1 bg-slate-900 text-white text-center py-4 rounded-2xl font-black text-xs">Ver</a>
...
const PeopleView: React.FC<any> = ({ students, setStudents, teachers, setTeachers, staff, setStaff, users, setUsers, showToast }) => {
  const [activeSub, setActiveSub] = useState<'students' | 'teachers'>('students');
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex gap-4">
        {['students', 'teachers'].map((s: any) => (
          <button key={s} onClick={() => setActiveSub(s)} className={`px-8 py-3 rounded-full font-black uppercase text-[10px] transition-all ${activeSub === s ? 'bg-slate-900 text-white' : 'bg-white text-slate-400 border border-slate-200'}`}>{s === 'students' ? 'Estudantes' : 'Professores'}</button>
        ))}
      </div>
      <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
        <form onSubmit={addItem} className="flex gap-4 mb-10">
          <input className="flex-1 px-8 py-5 rounded-2xl bg-slate-50 border border-slate-200 font-bold" value={newItem} onChange={e => setNewItem(e.target.value)} />
          <button className="bg-emerald-500 text-white px-10 rounded-2xl font-black uppercase shadow-lg transition-all"><Plus /></button>
        </form>
      </div>
    </div>
  );
...
const CalendarPlaceholder: React.FC = () => (
  <div className="bg-white p-20 rounded-[40px] shadow-sm border border-slate-100 text-center animate-in fade-in">
    <CalendarDays className="text-emerald-500 w-12 h-12 mx-auto mb-10" />
    <h3 className="text-4xl font-black text-slate-800 tracking-tighter mb-4">Calendário Integrado</h3>
    <p className="text-slate-400 font-bold">Em breve.</p>
  </div>
);
...
const PrintForm: React.FC<{ occurrence: Occurrence }> = ({ occurrence }) => (
  <div className="space-y-12 p-10 bg-white border-2 border-slate-100 rounded-3xl print:border-none print:p-0">
    <div className="flex justify-between items-center border-b-4 border-slate-900 pb-10">
      <div>
        <h2 className="text-4xl font-black text-slate-900 tracking-tighter italic uppercase">Guia de Ocorrência</h2>
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Protocolo #{occurrence.id.toUpperCase()}</p>
      </div>
    </div>
    <div className="space-y-10">
      <div><label className="text-[10px] font-black text-slate-400 uppercase mb-3 block">Estudante</label><p className="text-3xl font-black">{occurrence.student}</p></div>
      <div><label className="text-[10px] font-black text-slate-400 uppercase mb-3 block">Motivo</label><p className="text-lg font-bold bg-slate-50 p-8 rounded-3xl">{occurrence.reason}</p></div>
    </div>
    <div className="no-print mt-12 flex justify-center"><button onClick={() => window.print()} className="bg-slate-900 text-white px-12 py-6 rounded-3xl font-black flex items-center gap-4 shadow-2xl transition-all"><Printer size={24} /> Imprimir</button></div>
  </div>
);
...
const StudentHistoryView: React.FC<{ student: string; occurrences: Occurrence[] }> = ({ student, occurrences }) => {
  ...
  return (
    <div className="space-y-8">
      {filtered.length === 0 ? <p className="text-center py-12">Sem registros.</p> : filtered.map(o => (
        <div key={o.id} className="bg-slate-50 p-8 rounded-[40px] border border-slate-200">
          <div className="flex justify-between mb-6">
            <span className="px-4 py-1.5 rounded-full bg-slate-900 text-white text-[10px] font-black">{o.type}</span>
            <p className="font-black text-slate-400 text-[10px]">{new Date(o.date).toLocaleDateString()}</p>
          </div>
          <p className="font-bold text-slate-800 text-lg leading-relaxed">{o.reason}</p>
        </div>
      ))}
    </div>
  );
...
const NavBtn: React.FC<{ icon: any; label: string; active: boolean; onClick: () => void; isOpen: boolean }> = ({ icon: Icon, label, active, onClick, isOpen }) => (
  <button onClick={onClick} className={`flex items-center gap-4 w-full p-4 rounded-2xl transition-all duration-300 ${active ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 active:scale-95' : 'text-slate-500 hover:bg-slate-800 hover:text-white'}`}>
    <Icon size={20} />
    {isOpen && <span className="font-black text-xs uppercase tracking-widest">{label}</span>}
  </button>
);

const StatCard: React.FC<{ title: string; value: any; sub: string; icon: any; color: string }> = ({ title, value, sub, icon: Icon, color }) => {
  const colors: any = { emerald: 'text-emerald-500 bg-emerald-50', rose: 'text-rose-500 bg-rose-50', blue: 'text-blue-500 bg-blue-50', amber: 'text-amber-500 bg-amber-50' };
  return (
    <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 hover:shadow-xl transition-all active:scale-95 group">
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${colors[color]}`}><Icon size={20} /></div>
